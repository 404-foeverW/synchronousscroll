    ...
    <div class="main_scroll">
        <div class="synchronized-scrolling">
            ...
            <div class="table-wrapper left-wrapper">
                ...
                <div class="grid-body" ref="leftScrollEl" id="left-grid-body" @scroll="leftOnScroll">
                    <!-- JS 插入左侧数据 -->
            ...
            <!-- 表头区域 -->
                ...
                <!-- 表格内容区域 -->
                <div class="grid-body" id="grid-body" ref="rightScrollEl" @scroll="onScroll">
                    <!-- 数据将由 JS 插入这里 -->
...
let data = ref([]);
let leftData = ref([]);
// let targetScroll = ref(null);
let leftBoolScroll = ref(true);
let rightBoolScroll = ref(true);
let rightScrollEl = ref(null);
let leftScrollEl = ref(null);
const SCROLL_THROTTLE_TIME = 16;

let scrollBoolDebounce = debounce(initScrollBool, SCROLL_THROTTLE_TIME, false);
let scrollFuncThrottle = throttle(handleScroll, SCROLL_THROTTLE_TIME, true);
function leftOnScroll(e) {
    scrollFuncThrottle(e, rightBoolScroll, leftBoolScroll, rightScrollEl);
}
function onScroll(e) {
    scrollFuncThrottle(e, leftBoolScroll, rightBoolScroll, leftScrollEl);
}
function handleScroll(e, sourceBool, targetBool, targetEL) {
    if(!sourceBool.value) return;
    targetBool.value = false;
    if(targetEL.value) {
        targetEL.value.scrollTop = e.target.scrollTop;
    }
    scrollBoolDebounce();
}
function initScrollBool() {
    leftBoolScroll.value = true;
    rightBoolScroll.value = true;
}
onMounted(() => {
